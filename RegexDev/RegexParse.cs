...
namespace RegexDebug.RegexDev
{
	internal class RegexParse
	{
		...
		public IRegexNode ParseAST(string pattern, int style, out string patterndotNET5)
		{
				...
				if (prefix.Length > 0)
				{
					...
					else if (prefix[0] == ')')
					{
						...
						if (leftBracket.Type == RegexPrefixType.Condition)
						{
							...
							var c1 = listList[0][0] as RePanel;//must be, (?(?i)y|n) is illegal
							...
							if (c1.addBracket == true && c1.SequenceNodes.Count == 1 && c1.SequenceNodes[0] is ReSingle s && s.pattern.Length > 0 && IsWordChar(s.pattern[0]))
							{
								if (groupsHashSet.Contains(s.pattern))
									group = s.pattern;
								else if (s.pattern[0] >= '0' && s.pattern[0] <= '9')
								{
									if (s.pattern[0] == '0' && s.pattern.Length > 1 && s.pattern.All(a => a >= '0' && a <= '9') &&
										groupsHashSet.Contains(SourceGeneration.RemoveLeadingZerosRegex().Replace(s.pattern, ""))
										)
									{
										group = s.pattern;
									}
			...
			{
				//if conditon express directly contains regex options, the pattern need to be converted for .NET5+[issues#111633](https://github.com/dotnet/runtime/issues/111633)
	...
	internal partial class SourceGeneration
	{
		...
		/// <summary>
		/// replace leading 0 of string
		/// because BackreferenceConditional and \k'number' support numberic that begins with 0, so need to remove the leading zeros
		/// </summary>
		/// <returns></returns>
		[GeneratedRegex(@"^0+(?=\d)")]
		internal static partial Regex RemoveLeadingZerosRegex();
		...
		//[GeneratedRegex(@"(?ns)^(?>(?>(?'prefix'(?'open'\()(?(bracketX)(?'bracketX'))(?(bracketN)(?'bracketN'))(?>\?(?>'(?>(?'groupName'(?'onegroup'\w+))?(?'onegroup'-\w+)?)(?'-onegroup')(?'-onegroup')?'|<([=!]|(?>(?'groupName'(?'onegroup'\w+))?(?'onegroup'-\w+)?)(?'-onegroup')(?'-onegroup')?>)|[=!>:]|(?=\((?'-conditionExpress')?(?'-InvalidateNearestNumberedGroup')?([^?](?(openN)|(?'conditionExpress'))|(?'InvalidateNearestNumberedGroup')))|(?'openOption')(?>(?>\+(?'-openOption')?(?'openOption')|-(?'-openOption')?|[IMSims]+|[Xx]+(?(openOption)(?(openXOption)|(?'-closeXOption')?(?'openXOption'))|(?(closeXOption)|(?'-openXOption')?(?'closeXOption')))|[Nn]+(?(openOption)(?(openNOption)|(?'-closeNOption')?(?'openNOption'))|(?(closeNOption)|(?'-openNOption')?(?'closeNOption'))))+)(?>(?'-open'\))(?>(?'-bracketX')|)(?>(?'-bracketN')|)|:)(?'-openOption')?(?>(?'-openXOption')(?(openX)|(?(bracketX)(?(?<=(?!\k'bracketX')(?>(?!\().*?))(?'-bracketX')|(?<=(?'bracketX'\().+?))|(?<=(?'bracketX'\().+?))(?'openX'))|(?'-closeXOption')(?(openX)(?(?<=(?!\k'bracketX')(?>(?!\().*?))(?'-bracketX')|(?<=(?'bracketX'\().+?))(?'-openX'))|)(?>(?'-openNOption')(?(openN)|(?(bracketN)(?(?<=(?!\k'bracketN')(?>(?!\().*?))(?'-bracketN')|(?<=(?'bracketN'\().+?))|(?<=(?'bracketN'\().+?))(?'openN'))|(?'-closeNOption')(?(openN)(?(?<=(?!\k'bracketN')(?>(?!\().*?))(?'-bracketN')|(?<=(?'bracketN'\().+?))(?'-openN'))|))|(?=[^?])(?(openN)(?<=(?'groupNumber'.(?#used to mark current bracket's grouping number, capture empty content sign to have hnumber)))|(?>(?'-conditionExpress')(?<=(?'groupNumber'.))|(?'groupCount'(?#used to count how many numberic captured groups))(?>(?'-InvalidateNearestNumberedGroup')(?<=(?'groupNumber'.))|(?'groupNumber'))))))(?'q1')(?'q2')(?'InLineComment')|(?'prefix'\)(?'-open')(?(bracketX)(?>(?<=\k'bracketX')(?'-bracketX')|(?'-bracketX')(?'-bracketX')?((?'-openX')|(?'openX'))))(?(bracketN)(?>(?<=\k'bracketN')(?'-bracketN')|(?'-bracketN')(?'-bracketN')?((?'-openN')|(?'openN'))))|(?'open1'\[)(?>\^?)(?>(?'charrange'[]-](-(?>[^][\\]|\\([0-7]{1,3}|[^DdWwSsPpuxc]|u[0-9A-Fa-f]{4}|x[0-9A-Fa-f]{2}|c[@A-Za-z[\\\]^_])))?)?)(?>(?>(?'open1'-\[)(?>(?'-charrange')?\^?)(?>(?'charrange'[]-](-(?>[^][\\]|\\([0-7]{1,3}|[^DdWwSsPpuxc]|u[0-9A-Fa-f]{4}|x[0-9A-Fa-f]{2}|c[@A-Za-z[\\\]^_])))?)?)|((?>[^]\\]|\\([0-7]{1,3}|[^DdWwSsPpuxc]|u[0-9A-Fa-f]{4}|x[0-9A-Fa-f]{2}|c[@A-Za-z[\\\]^_]))(-(?>[^][\\]|\\([0-7]{1,3}|[^DdWwSsPpuxc]|u[0-9A-Fa-f]{4}|x[0-9A-Fa-f]{2}|c[@A-Za-z[\\\]^_])))?)|\\(?>[DdWwSs]|[Pp]\{[A-Za-z\d-]+\})|(?'-charrange'))+)(?>(?'-open1'\])+)(?(open1)(?!))|\\([dDwWsSAzZGbB]|k(?>'(?>[0-9]+|\w+)'|<(?>[0-9]+|\w+)>)|[0-9]+(?(openX)((?>[^[{+*?()\\|\t\r\n\f#]|\\[^Pp\dkcdDwWsSAzZGbBux])*(\{(?!\d+(,\d*)?\})|))*|((?>[^[{+*?()\\|\t\r\n\f]|\\[^Pp\dkcdDwWsSAzZGbBux])*(\{(?!\d+(,\d*)?\})|))*)|[Pp]\{[A-Za-z\d-]+\}|u([A-Fa-f0-9]{4})|x([A-Fa-f0-9]{2})|c([@A-Za-z[\\\]^_])))(?(openX)(?'InLineComment'\(\?#(?>[^)]*)\)|[ \t\r\n\f]+|#[^\r\n]*)*(?'q1'([*+?]|\{\d+(,\d*)?\})?)(?'InLineComment'\(\?#(?>[^)]*)\)|[ \t\r\n\f]+|#[^\r\n]*)*(?'q2'\??)|(?'InLineComment'\(\?#(?>[^)]*)\))*(?'q1'([*+?]|\{\d+(,\d*)?\})?)(?'InLineComment'\(\?#(?>[^)]*)\))*(?'q2'\??))(?'InLineComment')|(?'prefix'\|)(?'q1')(?'q2')(?'InLineComment')|(?(openX)(?>(?'prefix'((?>[^[+*?()\\|^$#]|\\[^Pp\dkcuxdDwWsSAzZGbB])(?!(?>(?>\(\?#(?>[^)]*)\)|[ \t\r\n\f]+|#[^\r\n]*)*)(?>[*+?]|\{\d+(,\d*)?\}))[ \t\r\n\f]*)+)(?'q1')(?'q2')(?'InLineComment')|(?>(?'prefix'[^+*?()\\|^$# \t\r\n\f]|\\.))(?>(?>(?'InLineComment'\(\?#(?>[^)]*)\)|[ \t\r\n\f]+|#[^\r\n]*)*)(?'q1'[*+?]|\{\d+(,\d*)?\})(?'InLineComment'\(\?#(?>[^)]*)\)|[ \t\r\n\f]+|#[^\r\n]*)*(?'q2'\??)(?'InLineComment')))|(?>(?'prefix'((?>[^[+*?()\\|^$]|\\[^Pp\dkcuxdDwWsSAzZGbB])(?!(?>(?>\(\?#(?>[^)]*)\))*)(?>[*+?]|\{\d+(,\d*)?\})))+)(?'q1')(?'q2')(?'InLineComment')|(?>(?'prefix'[^+*?()\\^$|]|\\.))(?>(?>(?'InLineComment'\(\?#(?>[^)]*)\))*)(?'q1'[*+?]|\{\d+(,\d*)?\})(?'InLineComment'\(\?#(?>[^)]*)\))*(?'q2'\??)(?'InLineComment'))))|(?'prefix'[$^])(?(openX)(?'InLineComment'\(\?#(?>[^)]*)\)|[ \t\r\n\f]+|#[^\r\n]*)*(?'q1'([*+?]|\{\d+(,\d*)?\})?)(?'InLineComment'\(\?#(?>[^)]*)\)|[ \t\r\n\f]+|#[^\r\n]*)*(?'q2'\??)|(?'InLineComment'\(\?#(?>[^)]*)\))*(?'q1'([*+?]|\{\d+(,\d*)?\})?)(?'InLineComment'\(\?#(?>[^)]*)\))*(?'q2'\??))(?'InLineComment')|((?'InLineComment'\(\?#(?>[^)]*)\))(?'prefix')(?'InLineComment')(?'q1')(?'q2'))+|(?(openX)|(?!))(?'InLineComment'#[^\r\n]*)(?'InLineComment'[ \t\r\n\f]+|\(\?#(?>[^)]*)\)|#[^\r\n]*)*(?'q1')(?'q2')(?'prefix')(?'InLineComment'))+)(?!(?'-open'))$")]
		[GeneratedRegex(@"(?ns)^(?>(?>(?'prefix'(?'open'\()(?(bracketX)(?'bracketX'))(?(bracketN)(?'bracketN'))(?>\?(?>'(?>(?'groupName'(?'onegroup'\w+))?(?'onegroup'-\w+)?)(?'-onegroup')(?'-onegroup')?'|<([=!]|(?>(?'groupName'(?'onegroup'\w+))?(?'onegroup'-\w+)?)(?'-onegroup')(?'-onegroup')?>)|[=!>:]|(?=\((?'-conditionExpress')?(?'-InvalidateNearestNumberedGroup')?([^?](?(openN)|(?'conditionExpress'))|(?'InvalidateNearestNumberedGroup')))|(?'openOption')(?>(?>\+(?'-openOption')?(?'openOption')|-(?'-openOption')?|[IMSims]+|[Xx]+(?(openOption)(?(openXOption)|(?'-closeXOption')?(?'openXOption'))|(?(closeXOption)|(?'-openXOption')?(?'closeXOption')))|[Nn]+(?(openOption)(?(openNOption)|(?'-closeNOption')?(?'openNOption'))|(?(closeNOption)|(?'-openNOption')?(?'closeNOption'))))+)(?>(?'-open'\))(?>(?'-bracketX')|)(?>(?'-bracketN')|)|:)(?'-openOption')?(?>(?'-openXOption')(?(openX)|(?(bracketX)(?>(?<=\k'bracketX')(?<=(?'bracketX'.).)|(?'-bracketX'))|(?<=(?'bracketX'.).))(?'openX'))|(?'-closeXOption')(?(openX)(?>(?<=\k'bracketX')(?<=(?'bracketX'.).)|(?'-bracketX'))(?'-openX'))|)(?>(?'-openNOption')(?(openN)|(?(bracketN)(?>(?<=\k'bracketN')(?<=(?'bracketN'.).)|(?'-bracketN'))|(?<=(?'bracketN'.).))(?'openN'))|(?'-closeNOption')(?(openN)(?>(?<=\k'bracketN')(?<=(?'bracketN'.).)|(?'-bracketN'))(?'-openN'))|))|(?=[^?])(?(openN)(?<=(?'groupNumber'.(?#used to mark current bracket's grouping number, capture empty content sign to have hnumber)))|(?>(?'-conditionExpress')(?<=(?'groupNumber'.))|(?'groupCount'(?#used to count how many numberic captured groups))(?>(?'-InvalidateNearestNumberedGroup')(?<=(?'groupNumber'.))|(?'groupNumber'))))))(?'q1')(?'q2')(?'InLineComment')|(?'prefix'\)(?'-open')(?(bracketX)(?>(?<=\k'bracketX')(?'-bracketX')|(?'-bracketX')(?'-bracketX')?((?'-openX')|(?'openX'))))(?(bracketN)(?>(?<=\k'bracketN')(?'-bracketN')|(?'-bracketN')(?'-bracketN')?((?'-openN')|(?'openN'))))|(?'open1'\[)(?>\^?)(?>(?'charrange'[]-](-(?>[^][\\]|\\([0-7]{1,3}|[^DdWwSsPpuxc]|u[0-9A-Fa-f]{4}|x[0-9A-Fa-f]{2}|c[@A-Za-z[\\\]^_])))?)?)(?>(?>(?'open1'-\[)(?>(?'-charrange')?\^?)(?>(?'charrange'[]-](-(?>[^][\\]|\\([0-7]{1,3}|[^DdWwSsPpuxc]|u[0-9A-Fa-f]{4}|x[0-9A-Fa-f]{2}|c[@A-Za-z[\\\]^_])))?)?)|((?>[^]\\]|\\([0-7]{1,3}|[^DdWwSsPpuxc]|u[0-9A-Fa-f]{4}|x[0-9A-Fa-f]{2}|c[@A-Za-z[\\\]^_]))(-(?>[^][\\]|\\([0-7]{1,3}|[^DdWwSsPpuxc]|u[0-9A-Fa-f]{4}|x[0-9A-Fa-f]{2}|c[@A-Za-z[\\\]^_])))?)|\\(?>[DdWwSs]|[Pp]\{[A-Za-z\d-]+\})|(?'-charrange'))+)(?>(?'-open1'\])+)(?(open1)(?!))|\\([dDwWsSAzZGbB]|k(?>'(?>[0-9]+|\w+)'|<(?>[0-9]+|\w+)>)|[0-9]+(?(openX)((?>[^[{+*?()\\|\t\r\n\f#]|\\[^Pp\dkcdDwWsSAzZGbBux])*(\{(?!\d+(,\d*)?\})|))*|((?>[^[{+*?()\\|\t\r\n\f]|\\[^Pp\dkcdDwWsSAzZGbBux])*(\{(?!\d+(,\d*)?\})|))*)|[Pp]\{[A-Za-z\d-]+\}|u([A-Fa-f0-9]{4})|x([A-Fa-f0-9]{2})|c([@A-Za-z[\\\]^_])))(?(openX)(?'InLineComment'\(\?#(?>[^)]*)\)|[ \t\r\n\f]+|#[^\r\n]*)*(?'q1'([*+?]|\{\d+(,\d*)?\})?)(?'InLineComment'\(\?#(?>[^)]*)\)|[ \t\r\n\f]+|#[^\r\n]*)*(?'q2'\??)|(?'InLineComment'\(\?#(?>[^)]*)\))*(?'q1'([*+?]|\{\d+(,\d*)?\})?)(?'InLineComment'\(\?#(?>[^)]*)\))*(?'q2'\??))(?'InLineComment')|(?'prefix'\|)(?'q1')(?'q2')(?'InLineComment')|(?(openX)(?>(?'prefix'((?>[^[+*?()\\|^$#]|\\[^Pp\dkcuxdDwWsSAzZGbB])(?!(?>(?>\(\?#(?>[^)]*)\)|[ \t\r\n\f]+|#[^\r\n]*)*)(?>[*+?]|\{\d+(,\d*)?\}))[ \t\r\n\f]*)+)(?'q1')(?'q2')(?'InLineComment')|(?>(?'prefix'[^+*?()\\|^$# \t\r\n\f]|\\.))(?>(?>(?'InLineComment'\(\?#(?>[^)]*)\)|[ \t\r\n\f]+|#[^\r\n]*)*)(?'q1'[*+?]|\{\d+(,\d*)?\})(?'InLineComment'\(\?#(?>[^)]*)\)|[ \t\r\n\f]+|#[^\r\n]*)*(?'q2'\??)(?'InLineComment')))|(?>(?'prefix'((?>[^[+*?()\\|^$]|\\[^Pp\dkcuxdDwWsSAzZGbB])(?!(?>(?>\(\?#(?>[^)]*)\))*)(?>[*+?]|\{\d+(,\d*)?\})))+)(?'q1')(?'q2')(?'InLineComment')|(?>(?'prefix'[^+*?()\\^$|]|\\.))(?>(?>(?'InLineComment'\(\?#(?>[^)]*)\))*)(?'q1'[*+?]|\{\d+(,\d*)?\})(?'InLineComment'\(\?#(?>[^)]*)\))*(?'q2'\??)(?'InLineComment'))))|(?'prefix'[$^])(?(openX)(?'InLineComment'\(\?#(?>[^)]*)\)|[ \t\r\n\f]+|#[^\r\n]*)*(?'q1'([*+?]|\{\d+(,\d*)?\})?)(?'InLineComment'\(\?#(?>[^)]*)\)|[ \t\r\n\f]+|#[^\r\n]*)*(?'q2'\??)|(?'InLineComment'\(\?#(?>[^)]*)\))*(?'q1'([*+?]|\{\d+(,\d*)?\})?)(?'InLineComment'\(\?#(?>[^)]*)\))*(?'q2'\??))(?'InLineComment')|((?'InLineComment'\(\?#(?>[^)]*)\))(?'prefix')(?'InLineComment')(?'q1')(?'q2'))+|(?(openX)|(?!))(?'InLineComment'#[^\r\n]*)(?'InLineComment'[ \t\r\n\f]+|\(\?#(?>[^)]*)\)|#[^\r\n]*)*(?'q1')(?'q2')(?'prefix')(?'InLineComment'))+)(?!(?'-open'))$")]
